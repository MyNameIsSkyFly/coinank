...
class _FrAlertView extends StatelessWidget {
  const _FrAlertView(this.logic);
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final list = logic.userAlerts
          .where((p0) => p0.type == NoticeRecordType.frAlert)
          .toList();
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            ...
                            ': ${Decimal.parse('${item.noticeValue}') * Decimal.fromInt(100)}% ${getTypeString(item.noticeType)}',
...
class _FrAlertSettingView extends StatefulWidget {
  const _FrAlertSettingView(this.logic);

  final AlertManageLogic logic;

  @override
  State<_FrAlertSettingView> createState() => _FrAlertSettingViewState();
}

class _FrAlertSettingViewState extends State<_FrAlertSettingView> {
  ...
  void _save() {
    ...
    Apis()
        .saveOrUpdateUserAlertConfigs(
      type: NoticeRecordType.frAlert,
      subType: _isUp.value ? 'up' : 'down',
      noticeValue: (alertValue / Decimal.fromInt(100)).toDouble(),
            ...
            return _dropDownButton(
              ...
              onTap: () async {
                final result = await showCupertinoModalPopup(
                  context: context,
                  builder: (context) => const _FrAlertSymbolSelector(),
                );
...
class _FrAlertSymbolSelector extends StatefulWidget {
  const _FrAlertSymbolSelector();

  @override
  State<_FrAlertSymbolSelector> createState() => _FrAlertSymbolSelectorState();
}

class _FrAlertSymbolSelectorState extends State<_FrAlertSymbolSelector> {
