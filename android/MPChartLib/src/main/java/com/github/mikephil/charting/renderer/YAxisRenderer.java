 ...
 */
            String extlabel = l.getExtLabel();
            String label = l.getLabel();

            if (extlabel == null || extlabel.equals(""))
            {
                limitLinePath.moveTo(mViewPortHandler.contentLeft(), pts[1]);
                limitLinePath.lineTo(mViewPortHandler.contentRight(), pts[1]);
            ...
            {
                float wlabel = Utils.calcTextWidth(mLimitLinePaint, label);
                float wextlabel = Utils.calcTextWidth(mLimitLinePaint, extlabel);
                wlabel = (wlabel > wextlabel) ? wlabel:wextlabel;

                float xOffset = Utils.convertDpToPixel(4f) + l.getXOffset();
                float right = mViewPortHandler.contentRight() - 10 - xOffset - wlabel;
            ...
            if (label != null && !label.equals("")) {
                ...
                if (position == LimitLine.LimitLabelPosition.RIGHT_TOP) {
                    ...
                    if (extlabel != null && !extlabel.equals(""))
                    {
                        ...
                        RectF r = new RectF();
                        r.left = mViewPortHandler.contentRight() - 10 - xOffset - wlabel;
                        r.top = pts[1] - yOffset - 5;
                        r.bottom = r.top + labelLineHeight*3 + 5;
                        r.right = r.left + wlabel + 15;

                        mLimitLinePaint.setColor(l.getLabelBgColor());

                        c.drawRect(r, mLimitLinePaint);
