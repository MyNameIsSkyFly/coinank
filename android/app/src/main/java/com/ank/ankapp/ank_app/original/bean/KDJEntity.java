...
public class KDJEntity {
     ...
     */
    public KDJEntity(List<KLineItem> kLineBeens, int n, int ma1, int ma2) {
            ...
            for (int i = 0; i < kLineBeens.size(); i++) {
                ...
                if (i > 0) {
                    if (n == 0) {
                        ...
                        int t = i - n + 1;
                        Float[] wrs = getHighAndLowByK(t, i, (ArrayList<KLineItem>) kLineBeens);
                        high = wrs[0];
                        low = wrs[1];
                    }
                }
                if (high != low) {
                    rSV = (kLineItem.close - low) / (high - low) * 100;
                } else {
                    rSV = 0;
                }
                k = k * 2 / ma1 + rSV / ma2;
    ...
    private Float[] getHighAndLowByK(Integer a, Integer b, ArrayList<KLineItem> kLineBeens) {
        ...
        for (int i = a; i <= b; i++) {
            kLineItem = kLineBeens.get(i);
            high = Math.max(high, kLineItem.high);
            low = Math.min(low, kLineItem.low);
        }
