...
import com.ank.ankapp.ank_app.original.bean.ChartNode;
...
public class OffsetBarRenderer extends BarChartRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        int colorListSize = dataSet.getColors().size();
        final boolean isSingleColor = colorListSize == 1;
        if (isSingleColor) {
            mRenderPaint.setColor(dataSet.getColor());
        }

        //MLog.d("buffer size:" + buffer.size());
        int k = -1;
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (!isSingleColor) {
                ...
                if (data == null)
                {
                    //MACD 资金费率等这里判断颜色
                    if (entry.getY() > 0) {
                        mRenderPaint.setColor(dataSet.getColors().get(0));//green
                    } else {
                        mRenderPaint.setColor(dataSet.getColors().get(1));//red
                    }
                }
                else if (data instanceof IndexRenderParam)
                {
                    IndexRenderParam param = (IndexRenderParam) data;
                    if (param.indexType == ChartNode.ChartType.MACD)
                    {
                        if (entry.getY() > 0) {
                            mRenderPaint.setColor(dataSet.getColors().get(0));//green
                        } else {
                        ...
                        {
                            //判断是否空心显示
                            BarEntry lastBar = dataSet.getEntryForIndex(k - 1);
                            if (entry.getY() >= lastBar.getY())
                            {
                    ...
                    //成交量指标这里判断颜色
                    int i = (int) data;
                    mRenderPaint.setColor(colorListSize > 1 ? dataSet.getColors().get(i % colorListSize) : Color.BLACK);
                }
            }
    ...
    public void drawHighlighted(Canvas c, Highlight[] indices) {
        ...
        for (Highlight high : indices) {
            ...
            float yMinValue = mChart.getYChartMin();//added by listen 20220406
            yMax = getYPixelForValues(xp, yMinValue);//added by listen 20220406
            //MLog.d("y:" + y + " yMin:" + yMin + " yMax:" + yMax + "yMinValue：" + yMinValue);
            if (y >= 0 && y <= yMax)
            {//在区域内即绘制横线
                ...
                float yValue = getYValue(y);//added by listen 20220407
                String text = AppUtils.getFormatStringValue(yValue); //format.format(yValue);
                int txtWidth = Utils.calcTextWidth(mHighlightPaint, text);
                int txtHight = Utils.calcTextHeight(mHighlightPaint, text);
                float top = Math.max(0, y - txtHight / 2F - halfPaddingVer);//考虑间隙
                float bottom = top + txtHight + halfPaddingVer * 2;
                if (bottom > yMax) {
                    bottom = yMax;
                    top = bottom - txtHight - halfPaddingVer * 2;
                }
                //RectF rect = new RectF(xMax - txtWidth - halfPaddingHor * 2, top, xMax, bottom);
                RectF rect = new RectF(halfPaddingHor, top, halfPaddingHor * 3 + txtWidth, bottom);
                mHighlightPaint.setStyle(Paint.Style.FILL_AND_STROKE);//BY listen0613
                ...
                //c.drawText(text, xMax - txtWidth - halfPaddingHor, baseY, mHighlightPaint);
                c.drawText(text, rect.left + halfPaddingHor , baseY, mHighlightPaint);
                //绘制横线
                //c.drawLine(0, y, xMax - txtWidth - halfPaddingHor * 2, y, mHighlightPaint);
