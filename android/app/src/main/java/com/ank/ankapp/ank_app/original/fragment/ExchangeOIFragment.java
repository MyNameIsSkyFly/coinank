...
import com.ank.ankapp.ank_app.original.activity.SearchBaseCoinListActivity;
import com.ank.ankapp.ank_app.original.adapter.ChooserBaseCoinAdapter;
import com.ank.ankapp.ank_app.original.adapter.HomeAdapter;
import com.ank.ankapp.ank_app.original.bean.BaseCoinVo;
import com.ank.ankapp.ank_app.original.bean.JsonVo;
import com.ank.ankapp.ank_app.original.bean.OIChartMenuParamVo;
import com.ank.ankapp.ank_app.original.bean.OIVo;
import com.ank.ankapp.ank_app.original.dialog.MenuDialog;
import com.ank.ankapp.ank_app.original.dialog.base.BaseDialog;
import com.ank.ankapp.ank_app.original.language.LanguageUtil;
import com.ank.ankapp.ank_app.original.language.PrefUtils;
import com.ank.ankapp.ank_app.original.utils.MLog;
import com.ank.ankapp.ank_app.original.utils.OkHttpUtil;
import com.ank.ankapp.ank_app.original.widget.HomeListView;
import com.ank.ankapp.ank_app.original.widget.refresh.PullScrollView;
...
public class ExchangeOIFragment extends Fragment implements View.OnClickListener {

    public static ExchangeOIFragment getInstance(Bundle bundle) {

        ExchangeOIFragment fragment = new ExchangeOIFragment();
        if (bundle != null) {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        return inflater.inflate(R.layout.fragment_oi_detail, container, false);
    }
    ...
    private HomeAdapter<OIVo> mAdapter;
    protected JsonVo<List<OIVo>> mData = new JsonVo();
    ...
    {
        chart_title.setText(getContext().getResources().getString(R.string.s_exchange_oi)
        ...
        webFragment.mAgentWeb.getJsAccessEntrace().quickCallJs("setChartData",
                jsonData, "android", "openInterest", options);
    }

    public void loadOIData()
    {
        long t1 = System.currentTimeMillis();
        String url = String.format(Config.apiGetOIChartData, menuParamVo.baseCoin, menuParamVo.interval, menuParamVo.type);
        Global.getAnalytics(getActivity()).logEvent("user_action",
                Global.createBundle("loadOIData url:" + url, getClass().getSimpleName()));
    ...
    private void initData() {
        ...
        menuParamVo.exchange = "ALL";
        menuParamVo.type = "USD";
        menuParamVo.interval = "1d";
        ...
        coinsAdapter.setItemClickListener(new ChooserBaseCoinAdapter.OnItemClickListener() {
            ...
            public void onClick(View v, int pos) {
                ...
                Global.getAnalytics(getActivity()).logEvent("user_action",
                        Global.createBundle("click " + menuParamVo.baseCoin + " " + menuParamVo.interval, getClass().getSimpleName()));
            }
        });
    }
    ...
    private void loadData(int type) {
        ...
        String url = "";
        url = String.format(Config.apiGetExchangeOIList, menuParamVo.baseCoin);
        OkHttpUtil.getJSON(url, new OkHttpUtil.OnDataListener() {
            @Override
            public void onResponse(String url, Object obj, String json) {
                ...
                if (type == 0) {
                    ...
                    Gson gson = new GsonBuilder().create();
                    mData = gson.fromJson(json, new TypeToken<JsonVo<List<OIVo>>>() {
                    }.getType());
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.ll_search) {
        ...
        else  if (v.getId() == R.id.tv_exchange) {
            String exchangeArrs[] = new String[]{"ALL", "Binance", "Okex", "Bybit",
                    "CME","Bitget", "Bitmex", "Bitfinex", "Gate","Deribit","Huobi", "Kraken"};

            List<String> data = new ArrayList<>();
            for (int i = 0; i < exchangeArrs.length; i++)
            {
                data.add(exchangeArrs[i]);
            }
            ...
            new MenuDialog.Builder(getContext())
                    ...
                    .setListener(new MenuDialog.OnListener<String>() {
                        ...
                        public void onSelected(BaseDialog dialog, int which, String string) {
                            ...
                            if (!exchangeArrs[which].equalsIgnoreCase(menuParamVo.exchange))
                            {
                                menuParamVo.exchange = exchangeArrs[which];
                                tv_exchange.setText(menuParamVo.exchange);
                                updateChart();
                            }
                        }
            ...
            String exchangeArrs[] = new String[]{"15m","30m", "1h", "2h", "4h", "12h","1d"};
            List<String> data = new ArrayList<>();
            for (int i = 0; i < exchangeArrs.length; i++)
            {
                data.add(exchangeArrs[i]);
            ...
            new MenuDialog.Builder(getContext())
                    ...
                    .setListener(new MenuDialog.OnListener<String>() {
                        ...
                        @Override
                        public void onSelected(BaseDialog dialog, int which, String string) {
                            if (!exchangeArrs[which].equalsIgnoreCase(menuParamVo.interval))
                            {
                                menuParamVo.interval = exchangeArrs[which];
                                tv_interval.setText(menuParamVo.interval);
                                loadOIData();
                            }
                        }
            ...
            String exchangeArrs[] = new String[]{"USD", menuParamVo.baseCoin};
            List<String> data = new ArrayList<>();
            for (int i = 0; i < exchangeArrs.length; i++)
            {
                data.add(exchangeArrs[i]);
            ...
            new MenuDialog.Builder(getContext())
                    ...
                    .setListener(new MenuDialog.OnListener<String>() {
                        ...
                        @Override
                        public void onSelected(BaseDialog dialog, int which, String string) {
                            if (!exchangeArrs[which].equalsIgnoreCase(menuParamVo.type))
                            {
                                menuParamVo.type = exchangeArrs[which];
                                tv_type.setText(menuParamVo.type);
                                loadOIData();
                            }
                        }
